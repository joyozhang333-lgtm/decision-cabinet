...
- 私有 UI API 默认仅允许本机访问，并校验 loopback 或显式配置的 Host；浏览器来源默认只信任 `localhost:5173` 与 `127.0.0.1:5173`。若通过反向代理或局域网开放，必须设置 `CABINET_UI_API_KEY`，并将前端的精确 Origin 加入逗号分隔的 `CABINET_UI_ORIGINS`；远程页面会显示密钥输入框，密钥仅保存在该浏览器的 localStorage。
