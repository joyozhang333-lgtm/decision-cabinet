...
- Private UI APIs accept loopback clients by default and validate a loopback or explicitly configured Host. Browser access trusts only `localhost:5173` and `127.0.0.1:5173` unless configured otherwise. Remote access requires `CABINET_UI_API_KEY` plus the exact frontend origin in the comma-separated `CABINET_UI_ORIGINS` setting.
